...
def main() -> None:
    ...
    dfx = build_xgb_eval_summary(market=args.market, latest_only=args.latest_only)
    if dfx.empty:
        print(f"[run_evaluation] No eventxgb challenger runs found for market={args.market}")
    else:
        csv_path, md_path = write_xgb_eval_outputs(dfx, market=args.market, as_of_date=args.as_of_date)
        print("[run_evaluation] Wrote:", csv_path)
        print("[run_evaluation] Wrote:", md_path)
